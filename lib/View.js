...
var View = function (options) {
  ...
  if (typeof options.path != 'undefined') {
    ...
    // TODO: move these to separate 'getter' methods
  ...
  if (typeof options.present != 'undefined') {
  ...
  if (typeof options.inputEngine != 'undefined') {
    self.inputEngine = options.inputEngine;
  }

  if (typeof options.outputEngine != 'undefined') {
    self.outputEngine = options.outputEngine;
  }

  if (typeof options.input != 'undefined') {
    this.input = options.input;
  } else { this.input = 'html'; }
...
View.prototype.render = function (data, cb) {
  var self = this
    , render = self.inputEngine.render // TODO: replace with this.getInputEngine().render
    , renderCallback
    ;
  ...
  // TODO: replace "self.rendered = result" with "this.setRendered(result)"
  renderCallback = function renderCallback(err, result) {
  ...
  if (cb) {
    return render(self.template, data, renderCallback);
  }
  // TODO: Add optional output rendering, if selected
  // TODO: Replace with:
  // var rendered (move to top of function)
  // rendered = render(template, data);
  // this.setRendered(rendered);
  self.rendered = render(self.template, data);
